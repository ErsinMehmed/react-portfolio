...
const ProjectCard = ({ project, type, onClick }: ProjectCardProps) => {
  ...
  const shared = {
    ...
    className:
      "group flex h-full flex-col rounded-2xl border border-slate-200/80 bg-white p-5 text-left shadow-sm transition-shadow duration-200 hover:shadow-[0_22px_50px_-24px_rgba(27,74,120,0.4)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand focus-visible:ring-offset-2 dark:border-slate-800 dark:bg-slate-900 dark:hover:shadow-[0_22px_50px_-24px_rgba(0,0,0,0.5)] dark:focus-visible:ring-offset-slate-950",
  };
  ...
  const body = (
    <>
      ...
      <div className="mt-4 flex items-center justify-between border-t border-slate-100 pt-3 dark:border-slate-800">
        ...
        <span className="text-brand transition-transform duration-200 ease-out group-hover:translate-x-1">
          &rarr;
        </span>
      </div>
    </>
